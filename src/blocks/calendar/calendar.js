...
(function (window, $, undefined) {
    ...
    datepicker.prototype = {
      ...
      _triggerOnChange() {
        ...
        formattedDates = selectedDates
          .map((date) => _this.formatDate(_this.loc.dateFormat, date))
      ...
      _isHotKeyPressed() {
        ...
        for (const hotKey in hotKeys) {
          ...
          if (
            currentHotKey.every((key, i) => key == pressedKeys[i])
          ) {
  ...
  (function () {
    ...
    const button = '<span class="datepicker--button #{action}" data-action="#{action}" >#{label}</span>';
    const { datepicker } = $.fn;
    const dp = datepicker.Constructor;
